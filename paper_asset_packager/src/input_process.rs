...
fn get_screen_size(node: &gltf::Node) -> f64 {
    //helper variables
    let default_screen_size = 0.5;
    let key = "screen_size";
    let error_msg = format!("| Warning: no LOD screen size selected. Using {} instead.", default_screen_size);
    ...
    match node.extras() {
        Some(node_val) => {
            //get json
            match serde_json::from_str(node_val.get()) {
                Ok(v) => {
                    //try to get value from json
                    let a: serde_json::Map<String, Value> = v;
                    match a.get(key) {
...
pub fn process_glb(file_dir: &std::path::PathBuf) -> Option<common::ModelData> {
    ...
    //scene
    let scene = match gltf_data.0.default_scene() {
        Some(v) => v,
        None => return None
    };
    ...
    lods.reserve(gltf_data.0.nodes().len());
    ...
    for node in gltf_data.0.nodes() {
        ...
        for primitive in mesh.primitives() {
            ...
            //print material name
            println!("| | Processing mesh: {}", mat_name);
